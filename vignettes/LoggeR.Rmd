...
The default value for `r maxSpeed` is 300 knots. While this is obviously an unrealistic speed for any sea-going vessel, this proved to be the most appropriate for the Midnatsol data. This may e because of small errors in the time stamp, giving large errors in estimated speed for (accurate) positions very close in time. For the Fram dataset, this needs to be changed to a much lower value. You can experiment with different `r maxSpeed`, and check the effect in the interactive map that opens up in a browser window.   
...
Once the data have been imported and cleaned up, they can be displayed on an interactive map (similar to the map that displays when running the `r filterGPS` function). Currently only whale data can be shown, but the function will be expanded to also allow bird data to be displayed. The function can be run either separately for a specific data set, or for several datasets combined: 
...
To create a shapefile of all tables in a data list, you can use the function `r convert2spatial`:
