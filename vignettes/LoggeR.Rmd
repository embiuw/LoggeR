...
Once the data have been imported and cleaned up, they can be displayed on an interactive map (similar to the map that displays when running the *filterGPS* function). Currently only whale data can be shown, but the function will be expanded to also allow bird data to be displayed. The function can be run either separately for a specific data set, or for several datasets combined: 
